...
[![C++17](https://img.shields.io/badge/C++-14-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white)](https://isocpp.org/)
[![Qt 6](https://img.shields.io/badge/Qt-5.0+-41CD52?style=for-the-badge&logo=qt&logoColor=white)](https://www.qt.io/)
[![Architecture](https://img.shields.io/badge/Arch-x86%20%7C%20x64-20232A?style=for-the-badge&logo=windows&logoColor=white)]()
[![API](https://img.shields.io/badge/WinAPI-Native-0078D4?style=for-the-badge&logo=windows&logoColor=white)]()
[![License: MIT](https://img.shields.io/badge/License-MIT-F7DF1E?style=for-the-badge&logoColor=black)](LICENSE)
...
*Designed for extreme stability, bypassing the flaws of traditional batch scripts through deep Windows API integration.*
...
## 📑 Table of Contents
1. [🛑 Legal & Prerequisite Notice](#-legal--prerequisite-notice)
2. [📖 Philosophy: Why Native C++?](#-philosophy-why-native-c)
3. [⚙️ Comprehensive Feature Breakdown](#️-comprehensive-feature-breakdown)
   - [Option 1: ❄️ The "Freeze Trial" Method](#option-1-️-the-freeze-trial-method)
   - [Option 2: 🚀 Full Activation (DLL Injection)](#option-2--full-activation-dll-injection)
   - [Option 3: 🔄 Deep Factory Reset & Smart Cleaner](#option-3--deep-factory-reset--smart-cleaner)
   - [Option 4: 📡 In-App OTA Updater](#option-4--in-app-ota-updater)
4. [🛠️ Under the Hood (Technical Architecture)](#️-under-the-hood-technical-architecture)
5. [🚀 Compilation & Build Instructions](#-compilation--build-instructions)
6. [👨‍💻 Author & Connect](#-author--connect)

---

## 🛑 Legal & Prerequisite Notice

> **Read Before Use:** Please strictly refer to the [LEGAL_NOTICE.md](LEGAL_NOTICE.md) file before proceeding. This repository is provided strictly for educational purposes, reverse-engineering analysis of Windows permission structures, and personal system management. 
...
**The Solution:** This toolkit is engineered from the ground up using **Pure Windows API (WinAPI)** inside a multi-threaded **Qt 6 Framework**. It directly manipulates Access Control Lists (ACLs), escalates privileges via security tokens, and injects native DLLs directly into process memory without ever blocking the user interface.
...
1. **Background Initialization:** IDM provides a legitimate 30-day trial. It tracks this trial using dynamically generated, heavily obfuscated `CLSID` registry keys. The tool first kills any running IDM instances.
2. **Key Generation:** It silently launches the IDM engine in the background for a few seconds to force it to generate fresh, valid 30-day trial keys.
3. **WinAPI Deep Lock:** Utilizing `SetNamedSecurityInfoW`, the tool aggressively applies a strict `Deny-Everyone` ACL permission directly onto these specific registry paths.
4. **The Result:** IDM can no longer read or modify its own trial expiration counters. It is permanently frozen at 30 days.
...
**How it works technically:**
Modern iterations of IDM employ an internal, hardcoded blacklist mechanism. If you submit generic registration data, IDM verifies it online, wipes the registration block upon failure, and triggers a persistent warning loop. This option executes a sophisticated 2-Step Bypass:
...
### Option 3: 🔄 Deep Factory Reset & Smart Cleaner
...
3. **DACL Rewrite & Annihilation:** Resets the Discretionary Access Control List (DACL) permissions to `GRANT_ACCESS` on all known IDM registry trees. It then safely executes a recursive tree deletion across `HKCU\Software\DownloadManager` and hidden `HKCR\CLSID` keys.
4. **The Result:** Returns the IDM environment to a pristine, "Factory Ready" state.

### Option 4: 📡 In-App OTA Updater (Network Module)
...
* Securely communicates with the GitHub raw repository over `HTTPS` to fetch the latest `version.txt`.
...
* **Language:** C++14
* **Framework:** Qt 5 (Qt Core, Qt Gui, Qt Widgets, Qt Network)
* **System API Integration:** * `Advapi32.dll` (Registry and ACL manipulation)
  * `Kernel32.dll` (Process and Thread memory management)
  * `User32.dll` (UI interactions and Hooks)
* **Concurrency:** Heavy operations (like recursive registry deletion or network fetching) are offloaded to background threads (`QThread` / `QEventLoop`) to guarantee the main GUI remains buttery smooth and responsive.
...
1. **Qt Framework:** Version 6.0 or higher.
...
**Steps:**
1. Clone the repository: `git clone https://github.com/alisakkaf/IDM-Freezer.git`
2. Open the `IDM_Tool.pro` file in **Qt Creator**.
3. Run `qmake` to generate the Makefiles.
...
**Ali Sakkaf** *Software Developer | System Utilities & Reverse Engineering Enthusiast*

* 🌐 **Website:** [mysterious-dev.com](https://mysterious-dev.com/)
* 🐙 **GitHub:** [@alisakkaf](https://github.com/alisakkaf)
* 📘 **Facebook:** [AliSakkaf.Dev](https://www.facebook.com/AliSakkaf.Dev/)

---
<div align="center">
<i>If this toolkit helped you understand Windows API permissions or process management better, consider leaving a ⭐ on the repository!</i>
</div>
